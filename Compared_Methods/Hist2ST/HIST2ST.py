...
class Hist2ST(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, fig_size=112, label=None, 
                 dropout=0.2, n_pos=512, kernel_size=5, patch_size=7, n_genes=601, 
                 depth1=2, depth2=8, depth3=4, heads=16, channel=32, 
                 zinb=0.25, nb=False, bake=0, lamb=0, policy='mean', 
                ):
        super().__init__()
    ...
    def forward(self, patches, centers, adj, aug=False):
        ...
        patches = self.patch_embedding(patches) # [B*N, C, H, W]
        centers_x = self.x_embed(centers[:,:,0]) # [B, N, C]
