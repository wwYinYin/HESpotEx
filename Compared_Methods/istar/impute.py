...
from istar.impute_by_basic import get_gene_counts, get_embeddings, get_locs
from istar.utils import read_lines, read_string, save_pickle
from istar.image import get_disk_mask
from istar.train import get_model as train_load_model
# from reduce_dim import reduce_dim
# from istar.visual import plot_matrix, plot_spot_masked_image
...
def get_model(
        x, y, locs, radius, prefix, batch_size, lr,
        load_saved=False, epochs=None, device='cuda'):
...
def predict_infer(
    ...
    y_grp = np.concatenate([
        np.median([
            predict_single_out(mod, z, idx_list, name_list, y_range)
            for mod, z in zip(model_states, z_states)], 0)
        for z_states in z_states_batches])
    return y_grp
            
def infer(
        embs, cnts, locs, radius, batch_size, prefix,
