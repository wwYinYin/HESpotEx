...
def get_embeddings(img, pretrained=True, device='cuda'):
    ...
    if pretrained:
        model256_path = '/home2/mengfanyi/YW/HE_predict/istar/checkpoints/vit256_small_dino.pth'
        model4k_path = '/home2/mengfanyi/YW/HE_predict/istar/checkpoints/vit4k_xs_dino.pth'
    model = HIPT_4K(
...
def img_feature_main(wsi,device,use_cache=None):
    np.random.seed(0)
    torch.manual_seed(0)

    if use_cache:
        # cache_file = args.prefix + 'embeddings-hist-raw.pickle'
        cache_file = use_cache
