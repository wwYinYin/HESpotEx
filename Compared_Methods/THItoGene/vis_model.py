...
class THItoGene(pl.LightningModule):
    def __init__(self, patch_size=112, n_layers=4, n_genes=1000, dim=1024, learning_rate=1e-4, dropout=0.2, n_pos=512,
    ...
    def forward(self, patches, centers, adj):
        ...
        patches = self.relu(patches) # B*N, 16, 28, 28

        patches = self.caps_layer(patches) # B*N, 20, 64
        patches = patches.reshape(-1, self.caps, self.route_dim) # B*N, 20, 64
        centers_x = self.x_embed(centers[:, :, 0]).permute(1, 0, 2) # B, N, 2
        centers_y = self.y_embed(centers[:, :, 1]).permute(1, 0, 2) # B, N, 2
