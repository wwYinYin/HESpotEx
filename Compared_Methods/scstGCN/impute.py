...
import numpy as np
from scstGCN.utils import read_lines, read_string, save_pickle, load_pickle, load_tsv, load_image, get_disk_mask
from scstGCN.train import get_model as train_load_model
from scipy.spatial.distance import cdist
from scstGCN.model import scstGCN
...
def get_model(
        x, y, locs, radius, ori_radius, prefix, batch_size, lr,
...
def impute_internel(
    ...
    y_grp=predict_infer(h, w,
            model_states=model_list, x_batches=embs_batches,
            name_list=names, y_range=cnts_range,
            prefix=prefix, device=device, gra_size=gra_size)    
    return y_grp

def predict_infer(h,w,
        model_states, x_batches, name_list, y_range, prefix,
        device='cuda', gra_size=7):
...
def infer(
        embs, cnts, locs, radius, ori_radius,batch_size, prefix,
