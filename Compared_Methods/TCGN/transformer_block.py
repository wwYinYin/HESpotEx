...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_size: int = 92, num_heads: int = 4, dropout: float = 0.1,add_diag:bool=False,device='cuda:0'):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor,mask: torch.Tensor = None):
        ...
        if self.add_diag==True:
            added=torch.diag_embed(torch.ones(
                [energy.shape[0], energy.shape[1], energy.shape[2]])).to(self.device)
            # energy BxHxNxN
...
class Multi_head_self_attention_with_graph_output(nn.Module):
    def __init__(self,emb_size=768,num_heads=8,add_diag=False,device='cuda:0'):
    ...
    def forward(self, x: torch.Tensor, relative_pos: torch.Tensor, mask: torch.Tensor = None):
        ...
        if self.add_diag:
            energy=energy+torch.diag_embed(torch.ones([energy.shape[0],energy.shape[1],energy.shape[2]])).to(self.device)
...
class Multi_head_cross_attention_with_graph_output(nn.Module):
    def __init__(self,emb_size=768,num_heads=8,add_diag=False,device='cuda:1'):
    ...
    def forward(self, key: torch.Tensor, query: torch.Tensor):
        ...
        energy=energy / scaling
        if self.add_diag:
            energy=energy+torch.diag_embed(torch.ones([energy.shape[0],energy.shape[1],energy.shape[2]])).to(key.device)
...
class TransformerEncoderBlock(nn.Module):
    def __init__(self, emb_size=768, drop_p=0.1, forward_expansion=4, num_heads=8
                 ,attention_add_diag=False,device='cuda:0'):
        ...
        self.Attention = MultiHeadAttention(emb_size=emb_size, num_heads=num_heads, dropout=drop_p
                                            ,add_diag=attention_add_diag,device=device)
...
class Channel_Attention(nn.Module):
    def __init__(self,num_nodes,device):
        ...
        self.transformer=TransformerEncoderBlock(emb_size=num_nodes,num_heads=self.num_heads,attention_add_diag=True,device=device)
